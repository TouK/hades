 ...
 */
public class CancellableConnectionGetter implements Callable<Connection> {
    ...
    public CancellableConnectionGetter(Hades hades, String curRunLogPrefix, boolean failover) {
        this.hades = hades;
     ...
     *
     * @return connection
     * @throws ConnectionGettingException if an exception occured
     */
    public Connection call() throws ConnectionGettingException {
        if (cancelled()) {
        ...
        try {
            connection = hades.getConnection(failover, curRunLogPrefix);
        } catch (ConnectionGettingException e) {
            handleException();
            throw e;
        }
